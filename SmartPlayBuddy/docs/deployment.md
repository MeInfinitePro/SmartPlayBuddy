...
┌────────────────────────────────────────────────────────────────┐
│                     服务器端 Server（已有远程托管）                │
│   http://smtplay.cabyss.cn:8000      统一登录 / 用户认证（JWT）    │
│   ws://smtplay.cabyss.cn:2508/ws     设备连接 / 消息路由 / 房间    │
└───────┬───────────────────────────────┬────────────────────────┘
...
1. **HTTP 认证服务（:8000）**：统一登录页、`/api/user/auth/authorize`（返回登录页地址）、
   `/api/user/auth/callback`（回跳并携带 accessToken/refreshToken/expiresIn）、
   `/api/user/auth/refresh`（刷新 token）。
2. **WebSocket 路由服务（:2508）**：JWT 鉴权接入、设备注册（deviceName）、
...
server {
    ...
    location / {
        ...
        proxy_set_header Host $host;
        proxy_set_header X-Forwarded-Proto $scheme;   # 网关借此拼出正确的登录回跳
    }
}
```
