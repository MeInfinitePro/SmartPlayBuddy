...
客户端会自动登录并注册为 `client:{userId}:{deviceName}`。设备名解析规则
（client 与 Mod 两端**共用同一套**，见 `src/smartplaybuddy/device.py`）：

1. 显式指定的值（client 端即环境变量 `SMTPLAY_DEVICE_NAME`，如 `SMTPLAY_DEVICE_NAME=my-pc smtplay`）
2. 未设置环境变量 → **自动使用本机主机名**（如 `DESKTOP-ABC123`），保证总是有一个固定、可路由的名字

> 提示：同一账号下设备名需保持唯一；多台电脑各用各的主机名天然满足，无需额外配置。

### 4.2 启动星穹铁道 Mod

**Mod 与 client 在同一台电脑时，两端解析出的设备名一致，可省略 `--target-client`：**

```bash
python -m mods.starrail
```

跨机部署（Mod 不在游戏电脑上），或 client 设置了自定义 `SMTPLAY_DEVICE_NAME` 时，
显式指定 client 注册的那个名字：
...
# 跨机 / 多设备：覆盖 Mod 默认目标设备（默认已自动取本机主机名）
