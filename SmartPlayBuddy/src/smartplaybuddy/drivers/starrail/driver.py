...
def _install_stdout_shim():
    ...
    设置环境变量 STARRAIL_DEBUG=1 时，print 内容改写到 driver_debug.log，
    便于排查游戏流程耗时/卡在哪一步。
    """
    original = sys.stdout
    debug_file = None
    if os.environ.get("STARRAIL_DEBUG"):
        try:
            debug_file = open(
                os.path.join(_PLUGIN_ROOT, "driver_debug.log"), "a", encoding="utf-8"
            )
        ...
        def write(self, s):
            ...
            for ch in s:
                if self.at_line_start:
                    out.append(f"[{_dt.datetime.now().strftime('%H:%M:%S.%f')[:-3]}] ")
    ...
    class _StdoutShim:
        ...
        def write(self, s, *args, **kwargs):
            if debug_file is not None and s:
                _ticker.write(s)
    ...
    _ticker = _Ticker(debug_file) if debug_file is not None else None
