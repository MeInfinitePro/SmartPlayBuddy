...
from .device import ENV_DEVICE_NAME, resolve_device_name
            ...
            if msg.Type == "command":
                ...
                if isinstance(resp, dict) and resp.get("status") == "ok":
                    if "__data__" in resp:
                        ...
                        return
                    # 普通（无二进制）响应：把 result 回传给发起方
                    # （修复：此前该分支只记错误不返回，导致所有非二进制驱动操作
                    #   在 Mod/控制器侧表现为"等待响应超时"）
                    meta = self.Message(
                        Type="response",
                        Action=msg.Action,
                        To=msg.From,
                    ...
                    await self.conn.send(meta.to_json())
                    logger.debug(i18n.translate("client.response_sent", size=0, to=msg.From))
...
def main():
    ...
    async def start():
        ...
        # 设备名解析（与 mod 端共用同一规则，保证两端能对上）：
        #   显式环境变量 SMTPLAY_DEVICE_NAME > 本机主机名兜底。
        # 主机名兜底确保不配置任何环境变量也能注册一个固定、可路由的名字，
        # 而不是空串被服务端分配随机 UUID（外部无法按名定位）。
