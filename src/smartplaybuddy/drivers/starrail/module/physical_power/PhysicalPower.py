...
def clear_physical_power(power_num:int=1,selection:int=3,use_reserve:bool=False):
    ...
    pyautogui.press("f4")
    #切换到生存索引界面
    interface.interface_switch(1030,0)

    current_power = get_power_data()['data']['current_power']

    reserve_power = get_power_data()['data']['reserve_power']
...
def cost_power(power_num, img_path:str, selection:int=3):
    if not interface.wait_until(physical_power_img_path.joinpath("survival_index.png")):
        print("当前不在生存索引界面")
        interface.wait_and_click(daily_task_img_path.joinpath("exit.png"))
        return
