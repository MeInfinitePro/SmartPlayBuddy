...
class StarRailGameManager:
    ...
    def open_game(self, wait_for_login: bool = True, login_timeout: float = 60.0) -> bool:
        """
        打开游戏（智能启动/切换）
        ...
        # 等待登录界面并点击进入
        if wait_for_login:
            print(f"等待登录界面（超时: {login_timeout}秒）...")
            login_btn = self.common_img_path / "in_login.png"
            if not login_btn.exists():
                print(f"⚠ 登录按钮图片不存在: {login_btn}")
                return True  # 如果图片不存在，认为已进入
            if  interface.wait_until(self.common_img_path/"in_main.png",timeout=5):
                return True
            if interface.wait_and_click(str(login_btn), timeout=login_timeout):
                print("✓ 点击登录按钮成功")
                # 等待进入主界面
                main_btn = self.common_img_path / "in_main.png"
                if main_btn.exists():
                    if interface.wait_until(str(main_btn), timeout=30.0):
                        print("✓ 成功进入游戏主界面")
                        return True
                    else:
                        print("⚠ 未检测到主界面，但可能已进入")
                        return True
                return True
            else:
                print("✗ 未检测到登录界面，可能已进入游戏或登录按钮图片不匹配")
                return True  # 可能已经登录了
