...
参数：
    --operate        指令名（必填）：ping / status / daily_task / daily_task/reward /
                    power/query / clear_power
...
def main():
    ...
    parser.add_argument("--operate", required=True,
                        help="ping / status / daily_task / daily_task/reward / power/query / clear_power")
    ...
    parser.add_argument("--timeout", type=float, default=600.0, help="等待响应超时（秒）")
