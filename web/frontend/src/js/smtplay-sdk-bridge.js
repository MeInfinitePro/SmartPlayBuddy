...
export { Message, ModBridge as SmtplayWSBridge }
 ...
 * 让平台能确定无疑地分辨出业务消息。
 *
 * ⚠️ 字段名在 src/composables/useModBridge.ts 有一份镜像，改这里必须同步改那里。
 */
...
export class ModBridge {
  constructor() {
    if (_instance) {
      console.warn(
        '[smtplay-sdk] ModBridge 应当单例使用；重复创建会让前一个实例的标记失效，已返回原实例',
      )
      return _instance
    }
    ...
    window.addEventListener('message', (e) => {
      ...
      if (p.type === 'ws' && p.action === 'receive') {
        ...
        let msg
        try { msg = Message.fromRaw(p) } catch { return }
        if (msg.type === 'system' && msg.action === 'target_device') {
          this.targetDevice = msg.data?.deviceAddress ?? null
          this._emit(msg)
          return
        }
        this._emit(msg)
  ...
  send(message) {
    ...
    window.parent.postMessage({
      ...mark,
      payload: { type: 'ws', action: 'send', data: message.toJSON() },
    }, '*')
    if (message.binaryData) {
      window.parent.postMessage({
        ...mark,
        payload: { type: 'ws', action: 'send', data: message.binaryData },
      }, '*')
    }
  }
